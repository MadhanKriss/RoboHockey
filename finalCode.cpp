...
const int fEnablePin = 3;  // Digital (PWM) pin for motor speed control
const int sPin1 = 4;  // Digital pin 1
const int sEnablePin = 5;  // Dig0ital (PWM) pin for moto r speed control
int PWM=255;
int half=128;
...
void loop() {
    ...
    else if(JoyEvents.y1>134){
      direction = "backward";
      backward();
    }
    else if(JoyEvents.x1>136){
      direction = "right";
      right();
    }
    else if(JoyEvents.x1<120){
      direction = "left";
      left();
    }
    else if(JoyEvents.y2<100){
      direction = "halfforward";
      halfforward();
    }
    else if(JoyEvents.x2>200){
      direction = "halfright";
      halfright();
    }
    else if(JoyEvents.x2<100){
      direction = "halfleft";
      halfleft();
    }
    else if(JoyEvents.y2>200){
      direction = "halfbackward";
      halfbackward();
    }
    else if (JoyEvents.hat==6){
      direction = "rotateleft";
      rotateleft();
    }
    else if (JoyEvents.hat==2){
...
void halfforward(){
  analogWrite(fEnablePin, half);  // Example PWM value (0-255) for half-speed
  analogWrite(sEnablePin, half); 
  digitalWrite(fPin1, LOW);
  digitalWrite(sPin1, LOW);
}
void halfbackward(){
...
// Right motor= f and Left motor = s
void halfright(){
 analogWrite(fEnablePin,PWM);  // Example PWM value (0-255) for half-speed
  analogWrite(sEnablePin, 0); 
  digitalWrite(fPin1, LOW);
